...
# --- 1. 깔끔하고 신뢰감 있는 테마 설정 (블루/슬레이트 계열) ---
professional_theme = gr.themes.Soft(
    primary_hue="blue",
    neutral_hue="slate",
    font=[gr.themes.GoogleFont("Noto Sans KR"), "sans-serif"]
).set(
    body_background_fill="#F8FAFC", # 차분한 연회색 배경
    block_background_fill="white",
    block_border_width="1px",
    block_border_color="#E2E8F0",
    button_primary_background_fill="#2563EB", # 신뢰감을 주는 블루 컬러
    button_primary_background_fill_hover="#1D4ED8"
)

# --- 2. 분석 실행 함수 ---
def analyze_accident(school_level, location, time, acc_type, body_part, activity):
    ...
    # ---------------------------------------------------------
    # 🏛️ 전문적인 카드 스타일 리포트 출력
    # ---------------------------------------------------------
    report = f"""
<div style="background-color: #FFFFFF; padding: 30px; border-radius: 12px; border: 1px solid #CBD5E1; box-shadow: 0 4px 6px -1px rgba(0, 0, 0, 0.05);">
    <h3 style="color: #1E3A8A; margin-top: 0; border-bottom: 2px solid #E2E8F0; padding-bottom: 10px;"> 📊 [{school_level}] {location} - {time} 사고 분석 종합 리포트 </h3>
    
    <h4 style="color: #2563EB; margin-top: 20px;"> 📌 1. 핵심 예측 지표 </h4>
    ...
    <h4 style="color: #2563EB; margin-top: 20px;"> 🔍 2. 머신러닝 모델 해석 (SHAP) </h4>
    <p style="background-color: #F8FAFC; padding: 12px; border-radius: 6px; border-left: 4px solid #2563EB; line-height: 1.6;"> {shap_desc} </p>

    <h4 style="color: #2563EB; margin-top: 20px;"> 💡 3. 현장 맞춤형 예방 가이드 (Action Item) </h4>
    <ul style="line-height: 1.8;">
    ...
    <h4 style="color: #2563EB; margin-top: 20px;"> 🔗 4. 공식 안전 지침 및 매뉴얼 </h4>
    {guideline_link}
    <br>
...
# --- 3. UI 구성 (기본값 빈칸 처리 완료) ---
with gr.Blocks(theme=professional_theme) as demo:
    gr.Markdown("<h1 style='text-align: center; color: #1E3A8A;'> 🏛️ AI 기반 학교 안전사고 예방 및 대응 시스템 </h1>")
    gr.Markdown("<p style='text-align: center; color: #475569;'><b>[빅데이터 분석]</b> 학교 안전사고 통계 데이터 및 <b>[머신러닝 모델]</b> LightGBM·CatBoost 앙상블을 활용한 지능형 안전 관리 플랫폼</p>")
    gr.Markdown("---")
    
    with gr.Row():
        with gr.Column(scale=1):
            gr.Markdown("### 🔍 사고 조건 설정")
            school_level = gr.Dropdown(choices=["유치원", "초등학교", "중학교", "고등학교", "특수학교"], label="🎒 학교급", value=None)
            ...
            time = gr.Dropdown(choices=time_options, label="⏰ 사고시간", value=None)
            ...
            location = gr.Dropdown(choices=location_options, label="🏫 사고장소", value=None)
            ...
            acc_type = gr.Dropdown(choices=type_options, label="💥 사고형태", value=None)
            ...
            body_part = gr.Dropdown(choices=body_part_options, label="🩹 사고부위", value=None)
            ...
            activity = gr.Dropdown(choices=activity_options, label="⚽ 사고당시활동", value=None)
            
            run_btn = gr.Button("📊 분석 실행", variant="primary")
            
        with gr.Column(scale=2):
            result_display = gr.HTML("<div style='text-align: center; margin-top: 80px; color: #64748B; font-size: 1.1em;'>👈 좌측에서 조건을 설정한 후 <b>[분석 실행]</b> 버튼을 클릭해 주세요.</div>")

    run_btn.click(
        fn=analyze_accident, 
        inputs=[school_level, location, time, acc_type, body_part, activity], 
